#### select certain syn training data from both ID data and ood data; 
import os
import glob
import random
...
print('the adopted selected adj and noun path is:', selected_adj_noun_path)

data_to_analyses = 'syn' ## real
pre_path="/home/notebook/code/personal/S9052995/syn_pro/OpenOOD/data/images_largescale/"

text_path_syn="/home/notebook/code/personal/S9052995/syn_pro/OpenOOD/data/benchmark_imglist/imagenet/train_imagenet_idood_last1k_syn32_dedup.txt"
text_path_real="/home/notebook/code/personal/S9052995/syn_pro/OpenOOD/data/benchmark_imglist/imagenet/train_imagenet_idood_last1k_real32_dedup.txt"
...
selected_adj_file = selected_adj
for char in special_chars_list:
    selected_adj_file = [name.replace(char, "_") for name in selected_adj_file]
...
selected_noun_file = selected_noun
for char in special_chars_list:
    selected_noun_file = [name.replace(char, "_") for name in selected_noun_file]
...
classnames_file = classnames
for char in special_chars_list:
    classnames_file = [name.replace(char, "_") for name in classnames_file]

classnames = classnames + selected_adj + selected_noun


# classnames_file

selected_index = 1711  ##orange milkwort

selected_classname = classnames[selected_index]
selected_classname_prompt = "a photo of a " + selected_classname 
# prompts = ["a photo of a " + name + "." for name in classnames]
tokenized_prompts = clip.tokenize(selected_classname_prompt).cuda()

selected_lines_syn = []
# 打开文件并读取行
with open(text_path_syn, 'r') as file:
    for line in file:
        # 移除行尾的换行符并分割行以获取最后一个元素
        parts = line.strip().split()
        # 检查是否最后一个元素为 '1'
        if parts[-1] == str(selected_index):
            selected_lines_syn.append(pre_path + parts[0])
selected_lines_real = []
# 打开文件并读取行
with open(text_path_real, 'r') as file:
    for line in file:
        # 移除行尾的换行符并分割行以获取最后一个元素
        parts = line.strip().split()
        # 检查是否最后一个元素为 '1'
        if parts[-1] == str(selected_index):
            selected_lines_real.append(pre_path + parts[0])  
            # pdb.set_trace()     
import shutil
move_image = True
print(selected_classname)
if move_image:
    target_file = './classes_' + str(selected_index)
    syn_file = target_file + '/syn/'
    real_file = target_file + '/real/'
    os.makedirs(syn_file)
    os.makedirs(real_file)
    index = 0
    for source_path in selected_lines_syn:
        shutil.move(source_path, syn_file+ str(index)+'.jpg')
        index+=1
    index = 0
    for source_path in selected_lines_real:
        shutil.move(source_path, real_file+ str(index)+'.jpg')
        index+=1
...
class CLIP_selector(nn.Module):
    def __init__(self, clip_model, train_preprocess, val_preprocess, tokenized_prompts):
        ...
        total_iter = math.ceil(tokenized_prompts.size(0) / batch_size)
        text_feat_list = []
        for i in range(total_iter):
            ...
            if i == total_iter - 1:
                with torch.no_grad():
                    text_feat = self.text_encoder(tokenized_prompts[i*batch_size:, :])
                    text_feat = text_feat / text_feat.norm(dim=-1, keepdim=True)
            else:
                with torch.no_grad():
                    text_feat = self.text_encoder(tokenized_prompts[i*batch_size:(i+1)*batch_size])
                    text_feat = text_feat / text_feat.norm(dim=-1, keepdim=True) ## 100 * D
            text_feat_list.append(text_feat)
        self.text_features = torch.cat(text_feat_list, dim=0)
...
model, val_preprocess = clip.load('ViT-B/16', 'cuda', jit=False)

class_num=1000
all_class_num=2000
# img_path=path
clip_selector = CLIP_selector(model, val_preprocess, val_preprocess, tokenized_prompts)

import statistics

selected_ims = select_by_clip_thres_RS(selected_lines_syn, clip_selector)
print('syn', statistics.mean(selected_ims), statistics.stdev(selected_ims), max(selected_ims), min(selected_ims))

selected_ims_real = select_by_clip_thres_RS(selected_lines_real, clip_selector)
print('real', statistics.mean(selected_ims_real), statistics.stdev(selected_ims_real), max(selected_ims_real), min(selected_ims_real))
# pdb.set_trace()

# train_cls = sorted(glob.glob('{}/*'.format(img_path)))  ## [..., '/home/notebook/code/personal/S9052995/syn_image/ImageNet_sdxl_ood_2/guidance_scale7.5/9_noun_ostrich']
# shot=8
# threshold = 1/all_class_num
...
#     cls_ims = sorted(glob.glob(subpath + '/*'))
#     selected_ims = select_by_clip_thres_RS(cls_ims, shot, clip_selector, id_class, thres=threshold)
#     selected_dir[cate_id] = selected_ims
#     total += len(cls_ims)
#     selected += len(selected_ims)
#     # print(selected, total)
...
# for test_string in selected_adj_file:
#     cate_id = id_class
#     contained_elements = [s for s in files if s.split('_', 2)[-1] == test_string]
#     # if len(contained_elements) != 1:
#         # ipdb.set_trace()
...
# for test_string in selected_noun_file:
#     cate_id = id_class
#     contained_elements = [s for s in files if s.split('_', 2)[-1] == test_string]
#     if len(contained_elements) != 1:
#         # ipdb.set_trace()
