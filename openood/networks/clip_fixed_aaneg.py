...
from openood.networks.clip_for_wordnet_prepare import load_corpus_dataset
import ipdb
import pdb
import json, os
...
def get_selected_ood_text_list(dataset='imagenet', total_ood_num=1000, corpus_dataset='neglabel_wordnet'):
    print('the ID dataset is:', dataset)
    print('total_ood_num is:', total_ood_num)
    print('corpus dataset is:', corpus_dataset) ## neglabel_wordnet | vanilla_wordnet |  part_of_speech_tagging
    classnames = get_class_names(dataset)
    selected_noun_text, selected_adj_text = load_corpus_dataset(classnames, corpus_dataset)
    # #######################################
    ...
    # wordnet_processed_path = foot_path + corpus_dataset + '_' + dataset + '_cossim_dedup.pth'
...
# https://github.com/openai/CLIP/blob/main/notebooks/Prompt_Engineering_for_ImageNet.ipynb
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number, corpus):
    classnames = get_class_names(dataset) # imagenet --> imagenet class names
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    ...
    # mean_text /= mean_text.norm()
    # mean_text = mean_text.view(-1, 1) 
    selected_adj_text, selected_noun_text = get_selected_ood_text_list(dataset=dataset, total_ood_num=ood_number, corpus_dataset=corpus)
    ...
    # pdb.set_trace()
    return text_features_id_ood


class FixedCLIP_NegOODPrompt_Fullcorpus(nn.Module):
    def __init__(self, cfg):
        ...
        self.text_features = get_text_features_neg(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt, cfg.backbone.text_center, cfg.backbone.ood_number, cfg.backbone.corpus)
        ...
        ############################################ random select a fixed number of words from the whole dataset.
        # pdb.set_trace()  ## totally 270K
        indices = torch.randperm(self.text_features.size(1) - self.n_cls)[:cfg.backbone.ood_number]
        self.text_features = torch.cat((self.text_features[:, :self.n_cls], self.text_features[:, self.n_cls:][:, indices]), dim=1)
        # print('cfg.backbone.ood_number do not work, use all negative labels')

        self.text_features_all = self.text_features
