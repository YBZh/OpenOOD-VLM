...
# import clip
from openood.networks.clip import clip_localfeat as clip
...
class FixedCLIP_OODPrompt_Localfeat(nn.Module):
...
def get_selected_ood_text_list(total_ood_num=1000):
    ...
    selected_adj_noun_path = foot_path + 'selected_adj_noun_0.95_'+ str(total_ood_num) + '_dedup.pth'
    unselected_adj_noun_path = foot_path + 'unselected_adj_noun_0.95_' + str(total_ood_num) + '_dedup.pth'
...
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number):
    ...
    # mean_text /= mean_text.norm()
    # mean_text = mean_text.view(-1, 1) 
    selected_adj_text, selected_noun_text, unselected_adj_text, unselected_noun_text = get_selected_ood_text_list(total_ood_num=ood_number)
...
class FixedCLIP_NegOODPrompt_Localfeat(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        backbone = cfg.backbone.name # 'ViT-B/16'
        classnames = get_class_names(cfg.backbone.dataset) # imagenet 
