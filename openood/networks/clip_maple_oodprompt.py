...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init and (n_ctx) <= 4:
            ...
            if cfg.backbone.CSC:
                ...
                ood_ctx_vectors = torch.empty(OOD_NUM, n_ctx, ctx_dim, dtype=dtype) # 1*77*512
        ...
        self.ood_ctx = nn.Parameter(ood_ctx_vectors)  # to be optimized, num*77*512 | OOD_NUM*16*512
        ...
        ood_prompts = [prompt_prefix + " " + 'X' + "." for i in range(OOD_NUM)]
    ...
    def construct_prompts(self, ctx, prefix, suffix, label=None):
        ...
        all_ctx = torch.cat([ctx, self.ood_ctx], dim=0)

        prompts = torch.cat(
            [
                prefix,  # (dim0, 1, dim)
                all_ctx,  # (dim0, n_ctx, dim)
                suffix,  # (dim0, *, dim)
            ],
            dim=1,
        )
