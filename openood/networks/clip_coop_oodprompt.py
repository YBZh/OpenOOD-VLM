...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            if cfg.CSC:
                ...
                ood_ctx_vectors = torch.empty(OOD_NUM, n_ctx+1, ctx_dim, dtype=dtype) # OOD_NUM*77*512
            else:
        ...
        ood_tokenized_prompts = torch.cat([clip.tokenize(p) for p in ood_prompts]) # 1000*77
        with torch.no_grad():
        ...
        self.register_buffer("ood_token_suffix", ood_embedding[:, 1 + n_ctx + 1 :, :])  # CLS, EOS, 
    ...
    def forward(self):
        ctx = self.ctx # 16*512, parameters to learn.
        if ctx.dim() == 2:
        ...
        if self.class_token_position == "end":
            ...
            ood_prompts = torch.cat(
                [
                    ...
                    self.ood_ctx,     # (n_cls, n_ctx+1, dim)
...
class CoOp_OneOODPrompt(nn.Module):
